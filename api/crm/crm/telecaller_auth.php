...
function tcLogin(array $b): void {
    ...
    if (strlen($mobile) !== 10) jsonErr('Valid mobile required');
    if (strlen($pass) < 6)      jsonErr('Password required');
    ...
    if (!$tc)                                    jsonErr('Mobile not registered or account inactive');
    if ($tc['partner_status'] !== 'active')      jsonErr('Partner account is inactive');
    if (!password_verify($pass, $tc['password_hash'])) jsonErr('Incorrect password');
...
function tcVerify(): void {
    $tc = authenticateTelecaller();
    jsonOk(['valid' => true, 'telecaller' => $tc]);
}
...
function tcActivity(array $b): void {
    ...
    if (!in_array($type, $allowed)) jsonErr('Invalid activity type');
...
function authenticateTelecaller(): array {
    ...
    if (!$tc) jsonErr('Session expired. Please login again.', 401);
    return $tc;
}
